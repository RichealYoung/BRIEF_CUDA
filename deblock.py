...
def main(step_dir:str, index_a:float, index_b:float, thres:float):
    ...
    if not os.path.exists(save_dir):
        os.mkdir(save_dir)
    for path in os.listdir(decompressed_dir):
        if path != 'chunks':
            origin_name = path
    ...
    print(info)
    block_infos = os.listdir(chunks_dir)
    lines = []
    time_start = time.time()
    for block_info in block_infos:
        dhw = block_info.split('.')[0]
        z1, z2, y1, y2, x1, x2= [int(r) for r in dhw.split('_')]
        z2, y2, x2 = z2 - 1, y2 -1, x2 -1
