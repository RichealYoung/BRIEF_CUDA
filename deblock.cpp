...
std::vector<uint16_t> filter(uint16_t p2,uint16_t p1,uint16_t p0,uint16_t q0,uint16_t q1,uint16_t q2,uint16_t index_b) {
    ...
    result.push_back((uint16_t)clip(p1+deltap1,0,65535));
    result.push_back((uint16_t)clip(p0+delta0,0,65535));
    result.push_back((uint16_t)clip(q0-delta0,0,65535));
    result.push_back((uint16_t)clip(q1+deltaq1,0,65535));
    return result;  
}
...
void deblock(std::string step_dir, uint32_t index_a, uint32_t index_b, uint32_t thres) {
    ...
    std::string chunks_dir = step_dir + "/chunks";
    ...
    std::vector<string> block_infos = listdir(chunks_dir);
    // find lines
    ...
    for (uint16_t i=0; i < block_infos.size(); i++) {
        std::string dhw = split(block_infos[i], ".")[0];
        std::vector<string> zyx = split(dhw, "_");
        uint16_t z1 = atoi(zyx[0].c_str()), z2 = atoi(zyx[1].c_str()) - 1;
        uint16_t y1 = atoi(zyx[2].c_str()), y2 = atoi(zyx[3].c_str()) - 1;
        uint16_t x1 = atoi(zyx[4].c_str()), x2 = atoi(zyx[5].c_str()) - 1;
